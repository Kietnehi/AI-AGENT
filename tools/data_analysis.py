...
class DataAnalysisTool:
    ...
    def get_info(self) -> str:
        ...
        for col in self.df.columns:
            info += f"Cột '{col}':\n"
            info += f"  - Kiểu dữ liệu: {self.df[col].dtype}\n"
            info += f"  - Giá trị null: {self.df[col].isnull().sum()}\n"
            info += f"  - Giá trị unique: {self.df[col].nunique()}\n"
            
            if pd.api.types.is_numeric_dtype(self.df[col]):
                info += f"  - Min: {self.df[col].min()}\n"
                info += f"  - Max: {self.df[col].max()}\n"
                info += f"  - Mean: {self.df[col].mean():.2f}\n"
